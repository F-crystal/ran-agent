...
- 用户随微信本轮上传截图、图片、音频、视频或文档时，桥接层的入站消息缓冲（`inboundMessageBuffer`）会先做 turn 聚合：媒体消息先 hold，等文字引用到达后合并为一个逻辑轮次（immediate 30s 窗口 / deferred 2min intent TTL），OpenClaw 收到的是已合并的 text+media payload。外部媒体文件（如 `/tmp/weixin-sdk`）会被自动复制到可信入站目录（`debug/wechat/inbound/`），项目内 `.env`、状态、vault 等文件永远不会被提升为可信媒体。桥接层在消息里列出可传给 MiMo 的可信入站 `file_path` / 远程 `url` assets 时，必须调用 MCP 工具 `mimo_power__analyze` 来分析这些媒体（不是”优先调用”），把分析结果沉淀为会话媒体 artifact，后续”刚才那张图/那段语音/那个视频”等指代应优先参考最近媒体上下文。`url` 必须是远程 `http(s)` URL。只有 MiMo Token Plan 不可用（返回 `MIMO_TOKEN_PLAN_KEY_MISSING`/`EXPIRED`）、用户明确要求快速 OCR/ASR，或输入是社媒/平台链接时，才 fallback 到 `social_reader__read_social_post_deep` 或 `media_reader` MCP；`media_reader` 是统一 facade，默认由本地 PaddleOCR、DashScope `qwen3-vl-flash`、`qwen3-asr-flash` 和服务器 `ffmpeg`/`ffprobe` 支撑，底层 provider、yt-dlp、ffmpeg、OCR/ASR/VLM 细节不直接暴露给 OpenClaw。
