...
normalize_openclaw_project_paths() {
  python3 - "$CONFIG_FILE" "$ROOT_DIR" "${CLAUDE_NODE_DIR:-$(command -v claude || true)}" <<'PY'
import json
import sys
...
if isinstance(agents, dict):
    ...
    if isinstance(defaults, dict) and defaults.get("workspace") != root_dir:
        ...
        if isinstance(cli_backends, dict):
            ...
            if isinstance(claude_cli, dict) and claude_command and claude_cli.get("command") != claude_command:
...
if isinstance(plugins, dict):
    ...
    if isinstance(load, dict):
        ...
        if isinstance(paths, list):
            normalized = [expected_path if isinstance(item, str) and item.endswith("/.openclaw/extensions") else item for item in paths]
            if normalized != paths:
                load["paths"] = normalized
                changed = True
            if expected_path not in load["paths"]:
