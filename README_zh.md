...
所有配置在 `.env.local` 中（不提交到 Git）。关键变量：

| 变量 | 用途 |
|------|------|
| `ANTHROPIC_BASE_URL` | Claude 兼容 API 端点 |
| `ANTHROPIC_AUTH_TOKEN` | API 认证 Token |
| `OPENCLAW_GATEWAY_TOKEN` | OpenClaw 网关认证 |
| `DASHSCOPE_API_KEY` | DashScope Provider 密钥（VLM、ASR、图片生成） |
| `TAVILY_API_KEY` | 网页搜索 API 密钥 |
| `PERSONAL_AGENT_VISION_PROVIDER` | 视觉模型 Provider（默认 `dashscope-qwen-vl`） |
| `PERSONAL_AGENT_VISION_MODEL` | 视觉模型（默认 `qwen3-vl-flash`） |
| `PERSONAL_AGENT_ASR_PROVIDER` | 语音识别 Provider（默认 `dashscope-asr`） |
| `PERSONAL_AGENT_BILIBILI_ENABLED` | 启用 B站平台解析 |
| `PERSONAL_AGENT_XHS_ENABLED` | 启用小红书平台解析 |
| `PERSONAL_AGENT_YTDLP_PATH` | yt-dlp 二进制路径（B站提取用） |
| `PERSONAL_AGENT_FFMPEG_PATH` | ffmpeg 路径（视频处理用） |

完整配置参考见 `local_archive/docs/deployment/`。
