...
| S10 | COMPLETE | S9 | Inventory legacy scheduler, reminders, daily digest, external MCP/forum/RSS pollers and dispatchers; split polling facts from visible attention; build manifest/watermark; rehearse on a production copy. | The 19-row machine manifest gives every legacy component one disposition. A production SQLite/state copy at `2026-08-08T08:28:45.000Z` migrated 0→2 with zero business rows/effects; three historical reminders were suppressed, future reminders and watches were zero, 13 legacy external activities were staged paused, one pending outbound item was held for reconciliation, and 58 sent plus 65 ambiguous legacy outbox rows became receipt/no-resend evidence only. The local external-poll worker seam records one hash-bound Core fact after WorkRun authority and exposes no send operation. |
| S11 | COMPLETE | S10 | Synthetic acceptance: duplicate/missed ticks, DST, crash, legacy fence, ambiguous outcomes, restart no-resend, and gaming/focus suppression with delayed coalescing. | Fault tests prove clock rollback never duplicates or regresses an occurrence, six-hour downtime collapses to one latest occurrence with one aggregate skip event, a mid-transaction tick failure rolls back completely, and a DST-fold tick creates the repeated wall time exactly once; one real synthetic delivery crosses wake/Exchange/WorkRun-claim plus the B.2 effect path exactly once across reopen/replay and terminalizes a timeout as `ambiguous` without redispatch; `attentionValve.mjs` delays and coalesces noncritical results during gaming/focus/unknown presence, keeps ambient silent, bypasses only owner-allowlisted critical keys and explicit reminders, and recovers interrupted flushes without loss. Core suite 149 and valve suite 9 pass; every external effect used a synthetic target; the valve is not wired into production paths (S12 scope). |
