...
S0 facts/runtime selection
  ...
  -> S10 migration rehearsal (current)
...
| S10 | IN_PROGRESS | S9 | Inventory legacy scheduler, reminders, daily digest, external MCP/forum/RSS pollers and dispatchers; split polling facts from visible attention; build manifest/watermark; rehearse on a production copy. | Every legacy component has a recorded disposition; external pollers produce Core facts rather than direct sends; legacy jobs default to paused; no historical reminder is re-sent. |
| S11 | NOT STARTED | S10 | Synthetic acceptance: duplicate/missed ticks, DST, crash, legacy fence, ambiguous outcomes, restart no-resend, and gaming/focus suppression with delayed coalescing. | A deployable SHA exists; noncritical proactive results do not interrupt gaming/focus, internal facts remain available, and every external effect uses a synthetic target. |
