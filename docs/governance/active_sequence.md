...
| S3 | IN_PROGRESS | A known personal-memory fact completes one observable capture-and-recall path through `personal_memory`, while the independent read-only Ombre result remains observable. The model proposes semantic content plus an identifier; Node remains the deterministic valve and accepts it only when identifier, content class and payload format agree. Project/runtime facts continue to use governed documents, not Ombre; O2 and direct Ombre mutation remain off. |
| S4 | PENDING | Valid retained work is merged or deliberately rejected, temporary worktrees are removed, obsolete local/remote branches are closed, and the inactive O2 compatibility implementation is deleted after its runtime references are proven absent. The preserved root-worktree draft is not lost. |
