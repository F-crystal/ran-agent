...
- OpenClaw main config now registers `playwright`, `time`, `media_generation`, `media_reader`, and `social_reader` MCP:
  ...
  - `media_reader` is the only OpenClaw-visible media understanding facade. It exposes `extract_media_assets`, `analyze_image`, `transcribe_audio`, `analyze_video`, and `analyze_media_batch`; lower-level OCR/ASR/VLM/ffmpeg providers stay behind adapters. The default adapters use DashScope `qwen-vl-ocr-2025-11-20`, cost-focused `qwen3-vl-flash`, `qwen3-asr-flash`, and server `ffmpeg`/`ffprobe`; missing `DASHSCOPE_API_KEY` / `QWEN_API_KEY` or missing ffmpeg binaries return structured errors.
