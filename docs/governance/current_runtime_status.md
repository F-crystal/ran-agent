...
- Repo MCP config now also registers:
  ...
  - `mimo_power` via `scripts/start_mimo_power_mcp.sh`, exposing `mimo_power__analyze` as a strong MiMo Token Plan multimodal/long-context analysis tool.
- OpenClaw main config now registers `playwright`, `time`, `media_generation`, `media_reader`, `social_reader`, and `mimo_power` MCP:
  ...
  - `media_reader` is the only OpenClaw-visible media understanding facade. It exposes `extract_media_assets`, `resolve_platform_media`, `analyze_image`, `transcribe_audio`, `analyze_video`, and `analyze_media_batch`; lower-level OCR/ASR/VLM/ffmpeg/yt-dlp/platform-provider details stay behind adapters. The default adapters use local PaddleOCR for OCR, cost-focused DashScope `qwen3-vl-flash`, DashScope `qwen3-asr-flash`, and server `ffmpeg`/`ffprobe`; missing PaddleOCR, missing `DASHSCOPE_API_KEY` / `QWEN_API_KEY`, missing ffmpeg binaries, missing platform providers, or missing yt-dlp return structured errors.
