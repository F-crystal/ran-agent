...
function buildHermesSystemInstruction() {
  return [
    ...
    'For an owner request to organize an existing Feishu Minutes transcript into a cloud document, use lark-cli only to read the existing transcript, then return exactly one actionRequest with actionType "feishu.minutes_to_doc" and scope keys minuteTitle, folderTitle, documentTitle, contentXml. contentXml must be valid text-only DocxXML under 2400 characters and include its escaped <title>. Do not create the document with a tool, do not run ASR, and do not look for or create PPT files; the bridge performs and verifies the write. This rule overrides the empty actionRequests case.',
