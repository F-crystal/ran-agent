...
async function uploadVoiceToWeixin(params) {
	return uploadMediaToCdn({
		...params,
...
/**
* Send a native voice message downstream using a previously uploaded voice file.
* VoiceItem: media only. Payload uses the same CDN media reference shape as inbound voice messages.
*/
async function sendVoiceMessageWeixin(params) {
	const { to, text, uploaded, opts } = params;
	if (!opts.contextToken) {
...
async function sendWeixinMediaFile(params) {
	const { filePath, to, text, opts, cdnBaseUrl, mediaType } = params;
	const mime = getMimeFromFilename(filePath);
	const uploadOpts = {
		baseUrl: opts.baseUrl,
		token: opts.token
	};
	if (mediaType !== "file" && mime.startsWith("audio/")) {
		const voiceFilePath = await normalizeVoiceFileForWeixin(filePath, mime);
		logger.info(`[weixin] sendWeixinMediaFile: uploading voice filePath=${voiceFilePath} to=${to}`);
		...
		logger.info(`[weixin] sendWeixinMediaFile: voice upload done filekey=${uploaded.filekey} size=${uploaded.fileSize}`);
...
async function normalizeVoiceFileForWeixin(filePath, mime) {
	if (mime === "audio/silk" || filePath.toLowerCase().endsWith(".silk") || filePath.toLowerCase().endsWith(".slk")) {
		return filePath;
	}
	if (mime !== "audio/wav" && mime !== "audio/x-wav") {
		throw new Error(`native weixin voice send only supports wav or silk input, got ${mime}`);
	}
	const { encode, isSilk } = await import("silk-wasm");
	let inputPath = filePath;
	if (isExecutableAvailable("ffmpeg")) {
		const normalizedPath = await normalizeVoiceWavWithFfmpeg(filePath);
		if (normalizedPath) inputPath = normalizedPath;
	}
	const input = await fs$1.readFile(inputPath);
	if (isSilk(input)) {
		return inputPath;
	}
	try {
		const encoded = await encode(input, 0);
		const saved = await saveMediaBuffer(Buffer.from(encoded.data), "audio/silk", "outbound", 0, "voice.silk");
		return saved.path;
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		throw new Error(`voice silk encoding failed: ${message}. Install ffmpeg and ensure the generated audio can be normalized to mono 24000Hz pcm_s16le WAV before sending.`);
	}
}
...
async function normalizeVoiceWavWithFfmpeg(filePath) {
	const dir = path.join(MEDIA_TEMP_DIR, "outbound");
	await fs$1.mkdir(dir, { recursive: true });
	const outputPath = path.join(dir, `voice-normalized-${Date.now()}-${crypto.randomBytes(4).toString("hex")}.wav`);
...
const MEDIA_TEMP_DIR$1 = "/tmp/weixin-agent/media";
	...
	while (!abortSignal?.aborted) try {
		...
		if (resp.ret !== void 0 && resp.ret !== 0 || resp.errcode !== void 0 && resp.errcode !== 0) {
			if (resp.errcode === -14 || resp.ret === -14) {
				...
				errLog(`[weixin] session expired (errcode -14), pausing for ${Math.ceil(pauseMs / 6e4)} min. Please run \`npx weixin-acp login\` to re-login.`);
...
//#endregion
export { Bot, isLoggedIn, login, logout, start };
