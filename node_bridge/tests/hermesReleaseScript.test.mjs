...
function makeDeployServiceFixture(initialStates = {}) {
  ...
  copyFileSync(join(root, 'scripts', 'verify-runtime-service-identity.sh'), join(scripts, 'verify-runtime-service-identity.sh'));
  chmodSync(join(scripts, 'resolve-hermes-service-node.sh'), 0o755);
  chmodSync(join(scripts, 'verify-runtime-service-identity.sh'), 0o755);
  ...
  writeFileSync(join(bin, 'systemctl'), [
    ...
    'case "$1" in',
    '  show) case "$*" in',
    '    *"--property=User"*) require_loaded "$2"; cat "$SYSTEMCTL_STATE/$2.user" 2>/dev/null || printf "%s\\n" "${MOCK_SERVICE_USER:-ubuntu}" ;;',
    '    *"--property=Group"*) require_loaded "$2"; cat "$SYSTEMCTL_STATE/$2.group" 2>/dev/null || printf "%s\\n" "${MOCK_SERVICE_GROUP:-ubuntu}" ;;',
    '    *"--property=MainPID"*) require_loaded "$2"; cat "$SYSTEMCTL_STATE/$2.pid" 2>/dev/null || printf "%s\\n" "${MOCK_MAIN_PID:-123}" ;;',
    '    *) load_state "$2" ;;',
    '  esac ;;',
    '  is-active)',
...
function prepareSourceIdentityFixture(fixture) {
  writeFileSync(join(fixture.bin, 'id'), [
    '#!/bin/sh',
    'case "$*" in',
    '  "-u ran-agent"|"-g ran-agent") printf "%s\\n" 999 ;;',
    '  "-gn ran-agent") printf "%s\\n" ran-agent ;;',
    '  "-u ubuntu"|"-g ubuntu") printf "%s\\n" 1000 ;;',
    '  "-gn ubuntu") printf "%s\\n" ubuntu ;;',
    '  *) exec /usr/bin/id "$@" ;;',
    'esac',
    '',
...
test('release apply is a server-only, rollback-capable transaction that preserves runtime shape', () => {
  ...
  assert.match(deploy, /"\$\{SUDO\[@\]\}" env RAN_AGENT_RUNTIME_USER="\$RUNTIME_USER" RAN_AGENT_RUNTIME_GROUP="\$RUNTIME_GROUP"[\s\S]*\$GATE_DIR\/scripts\/hermes-release-gate\.sh" --all/);
  assert.match(deploy, /runuser --user "\$RUNTIME_USER" --group "\$RUNTIME_GROUP" -- \/usr\/bin\/env -i[\s\S]*RAN_AGENT_RELEASE_SOURCE_ROOT="\$GATE_DIR"[\s\S]*\$GATE_DIR\/scripts\/hermes-release-gate\.sh" --all/);
  ...
  assert.ok(gateRunner.indexOf('runuser --user "$RUNTIME_USER" --group "$RUNTIME_GROUP"') > -1);
  assert.ok(gateRunner.indexOf('runuser --user "$RUNTIME_USER" --group "$RUNTIME_GROUP"') < gateRunner.indexOf('RAN_AGENT_GATE_SKIP_PRIVILEGED_TESTS=1'));
  assert.match(gateRunner, /RAN_AGENT_GATE_SKIP_PRIVILEGED_TESTS=0 bash "\$GATE_DIR\/scripts\/hermes-release-gate\.sh" --all/);
...
test('custom live state executes prepare and start without writing into the staged repo', () => {
  const dir = mkdtempSync(join(tmpdir(), 'ombre-custom-state-'));
  ...
  writeFileSync(join(bin, 'patch-python'), [
    '#!/bin/sh',
    'case "$*" in *"sys.version_info"*) printf "3.12\\n"; exit 0;; *"-m pip check"*|*"-m pip install"*|*"import frontmatter"*) exit 0;; *"src/server.py"*) printf "runtime=%s|%s\\n" "$RAN_AGENT_STATE_DIR" "$OMBRE_BRAIN_HOME"; exit 0;; esac',
    'case "$1" in',
    '  -m)',
    '    [ "$2" = venv ] || exit 1',
    '    mkdir -p "$3/bin"',
  ...
  const env = {
    ...
    RAN_AGENT_OMBRE_PYTHON_BIN: join(bin, 'patch-python'),
  ...
  try {
    ...
    assert.match(output, new RegExp(`runtime=${state.replaceAll('/', '\\/')}\\|${home.replaceAll('/', '\\/')}`));
...
test('Node, Ombre, Lite, and Full reuse one existing runtime identity', () => {
  ...
  assert.match(apply, /RUNTIME_USER="\$\{RAN_AGENT_RUNTIME_USER:-ubuntu\}"/);
  assert.match(apply, /RUNTIME_GROUP="\$\{RAN_AGENT_RUNTIME_GROUP:-\$RUNTIME_USER\}"/);
  assert.match(readFileSync(join(root, 'node_bridge', 'start_node.sh'), 'utf8'), /exec "\$NODE_BIN" src\/index\.mjs/);
  assert.match(ombreUnit, /User=\$RUNTIME_USER/);
  assert.match(ombreUnit, /Group=\$RUNTIME_GROUP/);
  assert.match(systemdUnits, /User=\$RUNTIME_USER/);
  assert.match(systemdUnits, /Group=\$RUNTIME_GROUP/);
  assert.doesNotMatch(apply, /STEWARD_RUNTIME_(?:USER|GROUP)|--ensure-account|verify-ran-agent-runtime-identity/);
});

test('ordinary release snapshot excludes retired O2 state and retains active durable state', () => {
  const fixture = makeDeployServiceFixture();
  const liveState = join(fixture.dir, 'live-state');
  const retired = join(liveState, 'ombre-compat', 'queue.jsonl');
  const durable = join(liveState, 'deliveries', 'queue.jsonl');
  try {
    mkdirSync(join(retired, '..'), { recursive: true });
    mkdirSync(join(durable, '..'), { recursive: true });
    writeFileSync(retired, '{"state":"retired"}\n');
    ...
    assert.equal(existsSync(join(fixture.snapshot, 'files', '900', 'deliveries', 'queue.jsonl')), true);
    assert.equal(existsSync(join(fixture.snapshot, 'files', '900', 'ombre-compat')), false);
  } finally {
...
test('source identity accepts absent optional Ombre while keeping Node mandatory', () => {
  const fixture = makeDeployServiceFixture({
    'ran-agent-node.service': { load: 'loaded', active: 'active', enabled: 'enabled' },
    'ran-agent-ombre-brain.service': { load: 'not-found', active: 'inactive', enabled: 'disabled' },
  });
  const stage = join(fixture.dir, 'stage');
  const proc = join(fixture.dir, 'proc');
  try {
    prepareSourceIdentityFixture(fixture);
    ...
    runDeployServiceFixture(fixture, [
      ...
      '[ "$SOURCE_RUNTIME_USER:$SOURCE_RUNTIME_GROUP:$SOURCE_RUNTIME_UID:$SOURCE_RUNTIME_GID" = ran-agent:ran-agent:999:999 ]',
      'printf "%s\\n" not-found >| "$SYSTEMCTL_STATE/ran-agent-node.service.load"',
      'if (resolve_source_runtime_identity) 2>/dev/null; then exit 91; fi',
      'printf "%s\\n" loaded >| "$SYSTEMCTL_STATE/ran-agent-node.service.load"',
      'printf "%s\\n" loaded >| "$SYSTEMCTL_STATE/ran-agent-ombre-brain.service.load"',
      'printf "%s\\n" active >| "$SYSTEMCTL_STATE/ran-agent-ombre-brain.service.active"',
      'if (verify_source_runtime_identity_unchanged) 2>/dev/null; then exit 92; fi',
      'printf "%s\\n" ubuntu >| "$SYSTEMCTL_STATE/ran-agent-ombre-brain.service.user"',
      'printf "%s\\n" ubuntu >| "$SYSTEMCTL_STATE/ran-agent-ombre-brain.service.group"',
      'if (resolve_source_runtime_identity) 2>/dev/null; then exit 93; fi',
    ].join('\n'), {
      MOCK_SERVICE_USER: 'ran-agent',
      MOCK_SERVICE_GROUP: 'ran-agent',
...
test('rollback restores snapshot with absent optional Ombre', () => {
  const fixture = makeDeployServiceFixture({
    'ran-agent-python.service': { load: 'loaded', active: 'inactive', enabled: 'enabled' },
    'ran-agent-node.service': { load: 'loaded', active: 'active', enabled: 'enabled' },
    'ran-agent-hermes.service': { load: 'loaded', active: 'inactive', enabled: 'enabled' },
    'ran-agent-hermes-full.service': { load: 'loaded', active: 'inactive', enabled: 'enabled' },
    'ran-agent-ombre-brain.service': { load: 'not-found', active: 'inactive', enabled: 'disabled' },
  });
  const stage = join(fixture.dir, 'stage');
  try {
    prepareSourceIdentityFixture(fixture);
    writeFileSync(join(fixture.snapshot, 'runtime-source-identity'), 'ran-agent\tran-agent\t999\t999\n');
    ...
    assert.match(output, /rollback-complete/);
    assert.doesNotMatch(output, /rollback-incomplete/);
    const systemctlTrace = readFileSync(fixture.log, 'utf8');
    assert.doesNotMatch(systemctlTrace, /show ran-agent-ombre-brain\.service --property=(?:User|Group|MainPID)/);
...
test('explicit rollback keeps immutable candidate helpers after checking out a prior commit without the pruner', () => {
  ...
  try {
    ...
    runAsCheckoutOperator('bash', ['-c', [
      ...
      'resolve_source_runtime_identity() { SOURCE_RUNTIME_USER=ubuntu; SOURCE_RUNTIME_GROUP=ubuntu; SOURCE_RUNTIME_UID=1000; SOURCE_RUNTIME_GID=1000; }',
      'quiesce_runtime_services() { return 0; }',
      'restore_runtime_files() { return 0; }',
      'restore_state_migrations() { return 0; }',
      'restore_service_state() { return 0; }',
...
test('candidate staging fails closed on missing release assets and remains readable but immutable to ran-agent', () => {
  ...
  try {
    ...
    assert.equal(statSync(join(completeStage, 'scripts', 'verify-runtime-service-identity.sh')).mode & 0o222, 0);
...
test('preserve runtime shape prepares Ombre and starts recall before lite and full without requiring Hermes CLI', () => {
  ...
  writeFileSync(join(bin, 'systemctl'), [
    ...
    '    *"--property=User"*) printf "%s\\n" "${MOCK_SERVICE_USER:-$MOCK_RUNTIME_USER}" ;;',
    '    *"--property=Group"*) printf "%s\\n" "${MOCK_SERVICE_GROUP:-$MOCK_RUNTIME_GROUP}" ;;',
    '    *) printf "%s\\n" loaded ;;',
    '  esac ;;',
    '  is-active) exit 0 ;;',
    '  is-enabled) printf "%s\\n" disabled ;;',
  ...
  writeFileSync(join(bin, 'ombre-python'), '#!/bin/sh\ncase "$*" in *"sys.version_info"*) printf "3.12\\n";; esac\nexit 0\n');
  ...
  chmodSync(join(bin, 'ombre-python'), 0o755);
  ...
  try {
    ...
    assert.equal(existsSync(join(dir, 'systemd', 'ran-agent-node.service.d', '99-ombre-steward-identity.conf')), false);
    ...
    assert.throws(() => execFileSync('bash', [join(root, 'scripts', 'apply-hermes-runtime-split.sh'), '--preserve-runtime-shape'], {
      cwd: root,
      env: { ...baseEnv, MOCK_SERVICE_USER: 'ran-agent' },
      stdio: 'pipe',
    }), /Command failed/);
    assert.doesNotMatch(readFileSync(trace, 'utf8'), /restart /);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('projection publisher switches to a distinct validated runtime identity and fails closed without it', () => {
  ...
  writeFileSync(join(bin, 'id'), [
    ...
    'if [ "${ROOT_RUNTIME_USER:-0}" = 1 ] && [ "$#" -gt 1 ]; then case "$1" in -u|-g) printf "%s\\n" 0; exit 0;; esac; fi',
    'case "$1:$#" in',
    '  -u:1|-g:1) printf "%s\\n" 2000 ;;',
    '  -u:2|-g:2) printf "%s\\n" 1000 ;;',
    '  -gn:2) printf "%s\\n" runtime-group ;;',
  ...
  try {
    ...
    const mutation = join(dir, 'root-target-mutation');
    assert.throws(() => execFileSync('bash', ['-c', [
      'source scripts/apply-hermes-runtime-split.sh',
      'resolve_runtime_identity',
      `printf mutation > ${JSON.stringify(mutation)}`,
    ].join('\n')], {
      cwd: root,
      env: { ...env, ROOT_RUNTIME_USER: '1' },
      stdio: 'pipe',
    }), /Command failed/);
    assert.equal(existsSync(mutation), false);
    const deploy = readFileSync(join(root, 'scripts', 'deploy-hermes-release.sh'), 'utf8');
    const prerequisites = deploy.match(/require_apply_prerequisites\(\) \{([\s\S]*?)\n\}/)?.[1] || '';
    assert.ok(prerequisites.indexOf('resolve_target_runtime_identity') < prerequisites.indexOf('project_checkout_permissions repair'));
    assert.ok(prerequisites.indexOf('resolve_source_runtime_identity') < prerequisites.indexOf('project_checkout_permissions repair'));
    const applyFlow = deploy.slice(deploy.lastIndexOf('require_apply_prerequisites'));
    for (const mutationName of ['snapshot_runtime_state', 'quiesce_runtime_services', 'activate_candidate_checkout']) {
      assert.ok(applyFlow.indexOf('require_apply_prerequisites') < applyFlow.indexOf(mutationName));
    }
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
test('non-preserve runtime split refuses direct canonical mutation before any host change', () => {
  ...
  try {
    assert.throws(() => execFileSync('bash', [join(root, 'scripts', 'apply-hermes-runtime-split.sh')], {
      ...
      encoding: 'utf8',
      stdio: 'pipe',
    }), /canonical production targets require an immutable staged release/);
    assert.equal(existsSync(state), false, 'direct apply must fail before runtime directories are changed');
    assert.equal(existsSync(trace), false, 'direct apply must fail before systemd mutation');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('release gate has an all mode that invokes the named smoke matrix after isolated suites', () => {
  ...
  assert.match(hermesResolver, /hermes_runtime_artifact\.v1\.json/);
  ...
  assert.doesNotMatch(source.match(/privileged_test_skip_reason\(\)[\s\S]*?\n\}/)?.[0] || '', /hermesGatewayProviderBoundary/);
  assert.match(source, /hermesModelCutover\.test\.mjs\|searchHubApplyScript\.test\.mjs\)/);
...
test('non-root gate skips only root tooling and never skips provider checks', () => {
  const pythonBin = requiredGatePython();
  ...
  try {
    ...
    const flagged = buildStage(toolingStubs);
    stages.push(flagged);
    ...
    const flaggedBoundary = buildStage([boundaryStub]);
    stages.push(flaggedBoundary);
    const flaggedBoundaryFailure = runGate(flaggedBoundary, { RAN_AGENT_GATE_SKIP_PRIVILEGED_TESTS: '1' });
    assert.notEqual(flaggedBoundaryFailure.status, 0);
    assert.match(flaggedBoundaryFailure.stderr, /hermes_(runtime_resolver_required|v0_20_runtime_required|unified_runtime_required)/);
    ...
    assert.match(boundaryFailure.stderr, /hermes_(runtime_resolver_required|v0_20_runtime_required|unified_runtime_required)/);
...
test('release transaction snapshots the live production checkout before activating an immutable candidate', () => {
  ...
  assert.match(deploy, /for stage in quiesce_runtime_services restore_runtime_files restore_state_migrations restore_node_dependencies restore_code_revision restore_service_state/);
...
test('release Hermes service resolver returns the configured executable while the gate owns version policy', () => {
  ...
  const unifiedBin = join(runtime, 'v020', 'hermes');
  const legacyBin = join(runtime, 'v013', 'hermes');
  mkdirSync(join(runtime, 'v020'), { recursive: true });
  mkdirSync(join(runtime, 'v013'), { recursive: true });
  writeFileSync(unifiedBin, '#!/bin/sh\nprintf "Hermes Agent v0.20.0\\n"\n');
  writeFileSync(legacyBin, '#!/bin/sh\nprintf "Hermes Agent v0.13.0\\n"\n');
  chmodSync(unifiedBin, 0o755);
  chmodSync(legacyBin, 0o755);
  const unified = makeSystemctlFixture({ cat: `ExecStart=${unifiedBin} gateway run` });
  const legacy = makeSystemctlFixture({ cat: `ExecStart=${legacyBin} gateway run` });
  ...
  try {
    assert.equal(runResolver(unified), unifiedBin);
    assert.equal(runResolver(legacy), legacyBin, 'generic path resolution must not create a second version authority');
    assert.throws(() => runResolver(missing), /Command failed/);
  } finally {
    rmSync(runtime, { recursive: true, force: true });
    for (const fixture of [unified, legacy, missing]) rmSync(fixture.dir, { recursive: true, force: true });
  }
});

test('staged Hermes gate binds one service-managed unified v0.20 runtime and retired Full stays non-runnable', () => {
  const stage = mkdtempSync(join(tmpdir(), 'ran-agent-hermes-gate-stage-'));
  ...
  const writeContracts = (runtime, { pythonDigest = sha256(readFileSync(runtime.python)) } = {}) => {
    ...
    const treeSha256 = '2'.repeat(64);
    writeFileSync(join(governance, 'hermes_runtime_artifact.v1.json'), JSON.stringify({
      schemaVersion: 1,
      artifact: { tarGzSha256, treeSha256 },
      source: { version: '0.20.0' },
      python: { version: '3.12.13', executableSha256: pythonDigest },
      dependencies: { installed: { 'hermes-agent': '0.20.0' } },
    }));
    writeFileSync(join(governance, 'hermes_runtime_mutation.v1.json'), JSON.stringify({
      schemaVersion: 1,
      deploymentStatus: 'DEPLOYED',
      artifactManifest: {
        path: 'docs/governance/hermes_runtime_artifact.v1.json',
        tarGzSha256,
        treeSha256,
        installRoot: runtime.installRoot,
        executable: runtime.hermes,
      },
      topology: { after: { gateways: [{ unit: 'ran-agent-hermes.service', port: 8642 }] } },
      unitMutations: [{
...
case "$cmd" in
  show)
    unit="$1"; shift; prop=""
    for arg in "$@"; do case "$arg" in --property=*) prop="\${arg#--property=}" ;; esac; done
    case "$unit:$prop" in
      ran-agent-hermes.service:User) printf '%s\\n' ${JSON.stringify(user)} ;;
      ran-agent-hermes.service:Group) printf '%s\\n' ${JSON.stringify(group)} ;;
      ran-agent-hermes.service:MainPID) printf '%s\\n' ${JSON.stringify(mainPid)} ;;
      ran-agent-hermes.service:ExecStart) printf '%s\\n' 'ExecStart=${configuredHermes} gateway run' ;;
...
`);
    chmodSync(systemctl, 0o755);
    const fixture = { dir, path: systemctl, procRoot };
  ...
  try {
    ...
    writeContracts(valid, { pythonDigest: '0'.repeat(64) });
    assert.throws(() => runGateResolver(makeSystemctl(valid)), /Command failed/);

    writeContracts(missingPython);
    const missingPythonFixture = makeSystemctl(missingPython);
    rmSync(missingPython.python);
    assert.throws(() => runGateResolver(missingPythonFixture), /Command failed/);
