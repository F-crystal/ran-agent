...
for f in .env.local node_bridge/.env.local "$HERMES_HOME/.env" "$HERMES_HOME/profiles/ran-assistant/.env"; do
  if [ -f "$f" ]; then
    for key in UV_CACHE_DIR UV_TOOL_DIR SOCIAL_READER_XHS_BACKEND_TIMEOUT_MS SOCIAL_READER_XHS_GENERIC_FALLBACK_TIMEOUT_MS XHS_BACKEND_MCP_TIMEOUT_MS SOCIAL_READER_MCP_TIMEOUT_MS MEDIA_READER_MCP_TIMEOUT_MS PERSONAL_AGENT_MEDIA_DOWNLOAD_TIMEOUT_MS PERSONAL_AGENT_MEDIA_MAX_CONCURRENCY PERSONAL_AGENT_MEDIA_BATCH_TIMEOUT_MS PERSONAL_AGENT_MEDIA_PER_ITEM_TIMEOUT_MS XHS_GENERIC_FALLBACK_MIN_VERSION; do
