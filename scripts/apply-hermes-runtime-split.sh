...
main() {
  ...
  if [ "${SOCIAL_READER_GENERIC_FALLBACK_ENABLED:-true}" != "false" ]; then
    ...
    if bash "$REPO_ROOT/scripts/prepare-xhs-generic-fallback.sh" 2>&1; then
      log "XHS generic fallback prepared"
    else
      log "WARNING: XHS generic fallback preparation failed (non-blocking)"
    fi
  fi
