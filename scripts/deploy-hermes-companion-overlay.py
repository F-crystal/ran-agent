...
def candidate_files(repo: Path, candidate: str) -> dict[str, bytes]:
    return {path: candidate_blob(repo, candidate, path) for path in OVERLAY_PATHS}
...
def render_dropin(root: Path) -> bytes:
    lines = ["[Service]", "BindReadOnlyPaths="]
...
def call_memory_mcp(node_bin: Path, server: Path) -> None:
    ...
    if not accepted.get("ok") or not isinstance(accepted.get("source_status"), dict):
        raise OverlayError("query recall did not expose source_status")
...
def verify_overlay(repo: Path, root: Path, files: dict[str, bytes], baseline: dict, node_bin: Path, manifest: dict) -> dict:
    ...
    expected = {path: sha256_bytes(value) for path, value in files.items()}
    ...
    return {"hermes_pid": pid}


def restore_dropin(transaction: Path, previous_present: bool) -> None:
    previous = transaction / "previous-dropin.conf"
    if previous_present:
        atomic_write(DROPIN, previous.read_bytes(), 0o644)
    else:
...
def rollback(transaction: Path, state: dict, manifest: dict) -> None:
    ...
    restore_dropin(transaction, bool(state["previous_dropin_present"]))
...
def require_current_transaction(state: dict, current_dropin_sha256: str | None, current_digests: dict | None) -> None:
    ...
    elif current_digests is not None and current_digests not in (
...
def explicit_rollback(args: argparse.Namespace) -> None:
    ...
    require_current_transaction(state, current_dropin_sha256, current_digests)
...
def preflight(args: argparse.Namespace) -> dict:
    ...
    if not service_active(HERMES_UNIT) or not service_active(NODE_UNIT) or not port_open(8642) or port_open(8643):
    ...
    if {path: sha256_bytes(value) for path, value in files.items()} != candidate_contract:
...
def apply(args: argparse.Namespace) -> None:
    ...
    state = {
        ...
        "candidate_namespace_digests": {path: sha256_bytes(value) for path, value in files.items()},
